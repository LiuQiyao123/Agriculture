...
> **[页面目标]** 为技术和基层人员提供数据源头管理和数据上报的工具。

1.  **自动化设备管理:**
    - **描述:** 左侧为设备列表，右侧为地图。列表与地图联动。
    - **列表字段:** `设备ID`, `设备类型`, `所属乡镇`, `状态`(在线/离线/告警), `电量(%)`, `最后上报时间`。
2.  **人工数据上报:**
    - **描述:** 一个标准的Web表单页面。
    - **表单字段:** `上报人`, `联系电话`, `上报时间`, `所属乡镇` (下拉选择), `地块编号`(关联选择), `事件类型`(病害/虫害/灾情/农事), `详细描述`(文本域), `现场照片`(支持上传3张)。
    - **交互:** 点击提交后，弹出“提交成功，感谢您的贡献！”的提示。

#### 3.2.A 地图图层：传感器定位点标注（新增）
- **目标**: 在数据资源中心的地图上直观展示自动化设备/传感器的空间分布，支持从列表到地图、地图到列表的双向联动。
- **数据结构（示例）**:
  - `id: string` 设备ID
  - `name: string` 设备名称（可选）
  - `type: string` 设备类型（如：土壤湿度、气象站、摄像头等）
  - `status: 'online' | 'offline' | 'alarm'` 当前状态
  - `coordinates: [lng:number, lat:number]` 经纬度（WGS84）
  - `battery?: number` 电量百分比（可选）
  - `lastSeen?: string` 最近上报时间（ISO字符串，可选）
- **渲染规则（DOM Marker）**:
  - 使用自定义圆形标注，带内发光：
    - `online`: 科技绿 `#00ff7f`
    - `alarm`: 告警红 `#ff4d4f`（外圈轻微脉冲动画）
    - `offline`: 暗灰 `#667085`
  - 悬停显示 tooltip：`设备名称/类型` + `状态` + `最近上报`。
  - 点击 marker：
    - 地图 `flyTo(center, zoom≈15)` 定位
    - 触发父组件事件（打开“设备详情”对话框）
- **图层开关**:
  - 在地图右上角“图层控制”中加入“传感器点”开关，默认开启，关闭后隐藏所有标注。
- **聚合（可选V2）**:
  - 当缩放 < 10 时启用简单聚合：按近邻聚为一个圆点，显示数字（该层暂可不做，保留拓展接口）。
- **联动**:
  - 列表→地图：点击“自动化设备管理”表格行，地图定位并高亮对应标注。
  - 地图→列表：点击标注，表格高亮该行并滚动可见（V2）。
- **图例**:
  - 右下角小图例：显示三种状态颜色与含义。
